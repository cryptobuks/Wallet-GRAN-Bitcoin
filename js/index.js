...
const CONTRACT_STRING = '0x0DBdD4E1228732a2d6F66796261DCf40b33199E1';

$(document).ready(init);

function init() {
	$('#blockexplorer').attr('href', 'https://ropsten.etherscan.io/token/' + CONTRACT_STRING);
	$('#project-web-page').attr('href', '');
...
function sendTokens(to,amount) {
	contract.transfer(to,amount*(10**12), function(err, res) {
...
function setTxHistory() {
	var txs = [];
	$.getJSON('https://ropsten.etherscan.io/api?module=account&action=txlist&address=' + CONTRACT_STRING + '&startblock=0&endblock=99999999&apikey=SQQJW5RNPKY13ZENIGTUXQD2PCKS5425EA', function(data) {
						...
						if (from == account || to == account) {
							web3.eth.getBlock(res.blockHash, function(e,r) {
								addTx({
									...
									amount: web3.toDecimal(res.logs[0].data) / 10**12
								});
							});
						}
					}
...
function addTx(json) {
	var ref = 'https://ropsten.etherscan.io/tx/'+json.hash;
	var message = '';
	if (json.to == account) {
